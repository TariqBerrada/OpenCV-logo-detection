...
#Performs the same work as the main detector.py script but plots the frames with the corresponding detections on them instead of saving the coordinates to a file.
...
def detect_color(frame):
    ...
    low_red = np.array([0,0,25])
    high_red = np.array([167,157,255])
            ...
            if score < threshold:
                ...
                if min(coordinates[1:]) > 0 and coordinates[2] + coordinates[4] < lx and coordinates[1] + coordinates[3] < ly:
                    ...
                    if s > 1.6:
                        ...
                        cv2.rectangle((frame), (coordinates[1], coordinates[2]), (coordinates[1]+coordinates[3], coordinates[2]+coordinates[4]), (0, 128, 255),2)
    cv2.imshow("Logo Detector", frame)
    cv2.waitKey(1)


for i in range(n_frames):
    frame = cv2.imread(frames_folder + "\\" + frames_name + str(i) + ".jpg")
    get_boxes(frame, i)
